...
fun ServerFinderDestination(
    ...
    // NSD system signals → Events (replaces MainActivity NSD collection block)
    LaunchedEffect(Unit) {
    ...
    // QR scanner overlay (moved from MainUI)
                    ...
                    CameraUI(visibility = isReadingQR) { ip, port ->
                        ...
                        work {
                            runCatching {
                                ...
                                viewModel.addServer(ServerInfo(ip = ip, port = port))
                            }.onFailure {
